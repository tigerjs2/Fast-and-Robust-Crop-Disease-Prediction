...
class SAM2BoxDecoder512(nn.Module):
    """
    512x512 백본 출력(32x32)과 프롬프트 인코더의 정적 출력(64x64) 간의 
    ...
    def forward(self, image_embed, feat_s0_256c, feat_s1_256c, box_coords):
        ...
        #    입력된 512 기준 좌표를 내부적으로 1024 스케일로 매핑하여 정확도를 유지합니다)
        scaled_box_coords = box_coords * 2.0 
        ...
        # --------------------------------------------------------------------
        # [핵심 패치] image_embed 크기(32x32)에 맞춰 PE 및 Dense 텐서 강제 축소
        # --------------------------------------------------------------------
        target_size = image_embed.shape[-2:] # (32, 32)
        ...
        feat_s0_adapted = self.conv_s0(feat_s0_256c) # 128x128 텐서
        feat_s1_adapted = self.conv_s1(feat_s1_256c) # 64x64 텐서
...
def main():
    ...
    pte_box_decoder_path = "mobile_assets/sam2.1_t_box_decoder_512.pte"
    
    print("[Step 1] SAM 2.1 모델 로드 및 512 최적화 Box 디코더 생성...")
    ...
    # ------------------------------------------------------------------------
    # [입력 준비] 512x512 통과 특성맵 파싱
    # ------------------------------------------------------------------------
    print("[Step 2] 512x512 백본 텐서 파싱 중...")
    ...
    print(f"  -> 메인 임베딩 텐서 형태: {image_embed.shape}") # [1, 256, 32, 32]
    ...
    if feat_s0 is None or feat_s1 is None:
        raise RuntimeError("백본 출력에서 128x128 또는 64x64 해상도 텐서를 찾을 수 없습니다.")

    print(f"  -> 주입될 보조 특징맵 1 (128x128): {feat_s0.shape}")
    print(f"  -> 주입될 보조 특징맵 2 (64x64): {feat_s1.shape}")

    # 512 화면 기준 더미 바운딩 박스
    dummy_box = torch.tensor([[[50.0, 50.0, 450.0, 450.0]]], dtype=torch.float32)
